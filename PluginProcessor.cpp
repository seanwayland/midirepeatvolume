...
void WaylochilliAudioProcessor::processBlock (AudioSampleBuffer& buffer, MidiBuffer& midiMessages)
{
    ...
    
    {
        ...
        for (MidiBuffer::Iterator i (midiMessages); i.getNextEvent (m, time);)
        {
            ...
            if (m.isNoteOn() && (m.getNoteNumber() != lastplayed) )
            {
                int NewNote = m.getNoteNumber();
                lastplayed = m.getNoteNumber();
            ...
            else if (m.isNoteOn() && (m.getNoteNumber() == lastplayed) )
                
            {
                offsetamt = offset[voice];
                int NewNote = m.getNoteNumber() + m.getVelocity() - 50 ;
